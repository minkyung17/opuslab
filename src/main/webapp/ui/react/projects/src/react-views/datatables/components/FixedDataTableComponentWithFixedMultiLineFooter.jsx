 ...
 **/
  ...
  *
  ******************************************************************************/
 export default class FixedDataTableComponentWithFixedMultiLineFooter extends React.Component {
   ...
   **/
    ...
    * @param {String} name - name of column
    * @return {JSX} - Cells
    * Maybe move this out of this component and into AdminCompProposals
    *
    */
   getTotalsDisplay = (name) => {
    let totals = this.props.totals;
    let totalDisplay = [];
    for(let each of totals){
        if(each.name===name){
            let eachTotal = each.totals;
            if(eachTotal.length>1 && eachTotal[0].name!==null){
                for(let index in eachTotal){
                    if(name==="proposedAdministrativeNinths" || name==="approvedAdministrativeNinths"){
                        totalDisplay[index] = eachTotal[index].label+eachTotal[index].ninthsAmount+", "+util.reformatToMoneyDisplayValue(util.formatMoney(eachTotal[index].amount, 2));
                    }else{
                        totalDisplay[index] = eachTotal[index].label+util.reformatToMoneyDisplayValue(util.formatMoney(eachTotal[index].amount, 2));
                    }
                }
            }else{
                totalDisplay[0] = "Total:"
                totalDisplay[1] = util.reformatToMoneyDisplayValue(util.formatMoney(eachTotal[0].amount, 2));
            }
        }
    }
    return totalDisplay;
   ...
   getColumn = (columnConfig = {}, data = {}) => {
     ...
     let total = this.getTotalsDisplay(name);
     return (<Column key={`column-${name}`} {...{fixed, width, flexGrow: 1}}
       cell={cellContent} header={headerJSX} footer={
     ...
     return (
       <div className=" datatable_root ">
         <Table ref="datatable" touchScrollEnabled {...{rowHeight, headerHeight,
           width, height, onScrollEnd, onContentHeightChange, rowsCount}} footerHeight={130}>
           {columns}
         </Table>
