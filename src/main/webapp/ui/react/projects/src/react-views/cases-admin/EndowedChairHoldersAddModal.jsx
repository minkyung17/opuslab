...
import {FormInput, FormTextArea, FormSelect, FormDate, FormAutoComplete} from
 ...
 *
 ******************************************************************************/
export default class EndowedChairHoldersAddModal extends React.Component {
   ...
   **/
    componentWillReceiveProps(props) {
        ...
        if(props.isAddRankDisabled!==this.state.isAddRankDisabled){
            this.setState({isAddRankDisabled: props.isAddRankDisabled})
        }
        ...
        modalDropdownOptions.emplStatusList = props.Logic.dataTableConfiguration.emplStatusOptions;
   ...
   **/
    ...
    saveECHData = async () => {
        ...
        if(!errorData.hasErrors){
            try {
                ...
                if(modalData.employeeStatus != null){
                    modalData['employeeStatusId'] = modalDropdownOptions.emplStatusList[modalData.employeeStatus];
                }
                ...
                clonedModalData.endowedChairId = this.props.Logic.dataTableConfiguration.endowedChairId;                                        
                let endowedChairHolderData = await this.props.Logic.saveAddData(clonedModalData);                
                if(modalComments.commentsText != undefined && modalComments.commentsText.trim() != ''){
                    modalComments.endowedChairHolderAppointmentId = endowedChairHolderData;
                ...
                console.log(endowedChairHolderData);
    ...
    hideEndowedModal = () => {
        ...
        this.setState({modalComments, errorData, isAddRankDisabled: true, showAddData: false, isSaveButtonDisabled: true});
    ...
    onChangeSeries = async(e, field) => {
        ...
        if(e.target.value != ''){
            ...
            this.setState({isAddRankDisabled: false});
        }else{
            modalDropdownOptions.rankList = [];
            modalData.rankTypeId = '';   
            this.setState({isAddRankDisabled: true});
        }
  ...
  *
  **/
  getNamesForAutocomplete = async ({term: searchText}, autoCompleteResponse) => {
    let formattedResultsPromise = this.props.Logic.getFormattedNameSearchOptions(
      ...
      autoCompleteResponse(formattedResults);
  ...
  *
  **/
  onSelectPerson = async (event, value) => {
    //No name so go back
    let {modalData} = this.state;
    ...
    modalData.opusId = value.id;
    this.getOpusPersonInfo(value.id);    
    this.setState({isSaveButtonDisabled: false, modalData, selectedPerson, showAddData: true});    
  }  

  async getOpusPersonInfo(opusPersonId) {
    let {modalData, modalDropdownOptions} = this.state;
    let personInfo =  await this.props.Logic.getOpusPersonInfo(opusPersonId);
    let emplStatus = personInfo.appointeeInfo.employeeStatus;
    modalData.employeeStatus = emplStatus;
    let emplStatusList = modalDropdownOptions.emplStatusList;
    let emplStatusId = Object.keys(emplStatusList).find(key => emplStatusList[key] === emplStatus);
    modalData.employeeStatusId = emplStatusId;
    this.setState({modalData});
}
   /**
   *
   ...
   **/
    render() {
        let {showModal, modalData, modalComments, errorData, modalDropdownOptions, isSaveButtonDisabled, isAddRankDisabled, selectedPerson, showAddData, resultsMessage} = this.state;
        return(
        <div>
            ...
            <Modal className=" modal-lg " backdrop="static" show={showModal}
                ...
                <Body>
                    <h2 className="flush-top">Add Chair Holder</h2>
                    <div>
                        <label>
                            Who would you like to add as a Chair Holder?
                        </label>
                        <FormAutoComplete placeholder={'Search for a person by name (last, first) or UID'}
                            ref="autocomplete" options={this.getNamesForAutocomplete} autoCompleteUIOptions={{}}
                        ...
                        <table className=' table table-bordered table-responsive'>
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td className={' col-md-8 '}>
                                        <FormDate onChange={(e) => this.onChangeDate(e, "effectiveDate")} 
                                ...
                                <tr>
                                    ...
                                    <td className={' col-md-8 '}>
                                        <FormDate onChange={(e) => this.onChangeDate(e, "endDate")} 
                                ...
                                <tr>
                                    ...
                                    <td className={' col-md-8 '}>
                                        <FormSelect includeBlankOption={true} disabled={isAddRankDisabled}
