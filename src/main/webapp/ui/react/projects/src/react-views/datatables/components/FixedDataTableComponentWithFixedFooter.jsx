 ...
 *
 ******************************************************************************/
export default class FixedDataTableComponentWithFixedFooter extends React.Component {
  ...
  **/
  ...
  getTotal(name) {
    ...
    if(name==='emplId'){
      ...
      for(let each of totals){
        if(each.name===name){
          if (each.total===0) {
            // Jira #3139 If total adds up to 0 should be displayed as $0
            total = '$0';
          } else if(each.total>=0){
            // total = '$'+each.total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");;
            total = util.reformatToMoneyDisplayValue(util.formatMoney(each.total, 2));
          }
        }
      }
    }
  ...
  getColumn(columnConfig = {}, data = {}) {
    ...
    return (<Column key={`column-${name}`} {...{fixed, width, flexGrow: 1}}
      cell={cellContent} header={headerJSX} footer={<Cell style={{fontWeight: "bold"}}>{total}</Cell>}/>);
  }
    ...
    return (
      <div className=" datatable_root ">
        <Table ref="datatable" touchScrollEnabled {...{rowHeight, headerHeight,
          width, height, onScrollEnd, onContentHeightChange, rowsCount}} footerHeight={36}>
          {columns}
        </Table>
