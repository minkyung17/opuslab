...
import {searchCriteria} from "../constants/HiddenMismatchesConstants";
import * as util from "../../common/helpers/";

export default class HiddenMismatches extends DataTable {
  //Class variables
    rowData = [];
   ...
   **/
    ...
    this.dataTableConfiguration, {link_key = "link"} = {}) {
        ...
        for(let row of formattedRowData) {
            ...
            let {originalData: {unitMismatch, affiliationMismatch, titleMismatch, seriesMismatch,
          rankMismatch, stepMismatch, endDateMismatch, apptStatusMismatch}} = row;
            if(unitMismatch === "Y") {
                row.opusUnitHighlight = true;
                row.ucPathDeptCodeHighlight = true;
            }
            if(affiliationMismatch === "Y") {
                row.opusAffiliationHighlight = true;
                row.ucPathJobIndicatorHighlight = true;
            }
            if(titleMismatch === "Y") {
                row.opusTitleHighlight = true;
                row.ucPathTitleHighlight = true;
            }
            if(seriesMismatch === "Y") {
                row.opusSeriesHighlight = true;
                row.ucPathSeriesHighlight = true;
            }
            if(rankMismatch === "Y") {
                row.opusRankHighlight = true;
                row.ucPathRankHighlight = true;
            }
            if(stepMismatch === "Y") {
                row.opusStepHighlight = true;
                row.ucPathStepHighlight = true;
            }
            if(endDateMismatch === "Y") {
                row.opusEndDateHighlight = true;
                row.ucPathEndDateHighlight = true;
            }
            if(apptStatusMismatch === "Y") {
                row.opusApptStatusHighlight = true;
                row.ucPathJobStatusHighlight = true;
            }
        }
        return formattedRowData;
    }
 ...
 **/
   ...
   *
   **/
    async unhideRow(rowData, unhideFlag) {
        let {grouperPathText, access_token, adminData} = this;

        console.log("Unhide Row data being sent to backend:");
        console.log(rowData);
        let stringified = JSON.stringify(rowData);
        let unhideUrl = this.dataTableConfiguration.unhideUrl;
        unhideUrl = this.addAccessTokenAndGrouperToUrl(unhideUrl, access_token, {grouperPathText, addGrouper: true});
        unhideUrl = unhideUrl+"&appointmentId="+rowData.opusApptId+"&emplId="+rowData.emplId
                +"&emplRcd="+rowData.emplRcd+"&isHidden="+unhideFlag+"&opusScreenName=Hidden&grouperPathText="+grouperPathText;
        console.log(unhideUrl);
        return util.jqueryPostJson(unhideUrl, stringified);
    }
