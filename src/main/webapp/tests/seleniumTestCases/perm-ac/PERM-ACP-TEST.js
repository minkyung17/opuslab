...
const pageToTest = "AdminCompProposals";  // must use same key as used elsewhere (e.g. data/PAGE_PATHS)
...
exports.runPermACP = async (caller, environment, featureToTest, role, shouldSee) => {
    //////////////////////////////////////////////////////////////////
	// ROLE CHECK
	//////////////////////////////////////////////////////////////////
    
    // define prefix for output
    const testModule = "PERM-ACP";
    ...
    try
    {
		...
		try
		{
			...
			await selectedPage.checkForData(loggingPrefix, driver);
		}
		catch(error)
		{
		    pageDidLoad = false;
		...
		if(pageDidLoad)
		{
			...
			try
			{
			    if(featureToTest === "add")
			    {
					...
					await selectedPage.	clickAddNewProposalButton(loggingPrefix, driver);
			    ...
			    else if(featureToTest === "edit")
			    {
					...
					// new tab will open so switch to it
					await selectedPage.changeTab(loggingPrefix, driver, pageToTest);
					
					// get the admin comp ID from the URL so result can be checked
					let currentURL = await driver.getCurrentUrl();
					permACLib.logThis(loggingPrefix + "current URL is: " + currentURL);
					let urlParts = currentURL.split("=");
					let adminCompID = urlParts[1];
					permACLib.logThis(loggingPrefix + "actual admin comp ID in URL is: " + adminCompID);
					permACLib.logThis(loggingPrefix + "expected admin comp ID is:      " + shouldSee);
					
					// if click works without error, set raw result as having access
			        rawResult = adminCompID;
			    }
			    else if(featureToTest === "delete")
			    {
			        ...
			        rawResult = true ? rawResult = "access" : "denied";
			    }
			}
			catch(error)
			{
				// if button CAN'T be found, role doesn't have access
				permACLib.logThis(loggingPrefix + '"Add Proposal" button can\'t be found, tab change was unsuccessful, or modal didn\'t launch');
				rawResult = "denied";
