...
const pageToTest = "AdminCompProposalSummary";  // must use same key as used elsewhere (e.g. data/PAGE_PATHS)
...
exports.runPermACPEdit = async (caller, environment, featureToTest, role, shouldSee) => {
    ...
    const testModule = "PERM-ACP-EDIT";
    ...
    try
    {
		...
		if(pageDidLoad)
		{
			...
			try
			{
			    if(featureToTest === "proposed_edit")
			    {
					// click Proposed edit icon
					await selectedPage.clickProposedEditIcon(loggingPrefix, driver);
					
					// check for modal
				    await selectedPage.checkForModal(loggingPrefix, driver);
			    }
			    else if(featureToTest === "final_edit")
			    {
					// click Final Decision edit icon
					await selectedPage.clickFinalDecisionEditIcon(loggingPrefix, driver);
					
					// check for modal
				    await selectedPage.checkForModal(loggingPrefix, driver);
			    }
			    else if(featureToTest === "tracking")
			    {
			        // click Tracking Dates edit icon
			        await selectedPage.clickTrackingDatesEditIcon(loggingPrefix, driver);
			        
			        // check for modal
				    await selectedPage.checkForModal(loggingPrefix, driver);
			    }
			    else if(featureToTest === "proposed_comments")
			    {
                    // click Proposed comments icon
			        await selectedPage.clickProposedCommentsIcon(loggingPrefix, driver);
                    
                    // check for modal
				    await selectedPage.checkForModal(loggingPrefix, driver);
			    }
			    else if(featureToTest === "final_comments")
			    {
                    // click Final Decision comments icon
			        await selectedPage.clickFinalDecisionCommentsIcon(loggingPrefix, driver);
			...
			catch(error)
			{
				...
				permACLib.logThis(loggingPrefix + 'feature check failed or modal didn\'t launch');
				rawResult = "denied";
		...
		else if (caller === "OTTO")// it's otto/scheduler
		{
		    ...
		    xrayTestIssueKey = permACLib.getXRAYTestIssueKey(pageToTest);
