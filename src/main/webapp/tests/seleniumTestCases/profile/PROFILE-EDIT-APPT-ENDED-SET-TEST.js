...
exports.runProfileEditAppointmentEndedSetTest = async (caller, environment) => {
	// initialize values
	var passFail = "";
	const modalComments = "PROFILE-EDITAPPTENDEDSET";
	 
    // define prefix for output
    const testModule = "PROFILE-EDIT-APPT-ENDED-SET apo";
    ...
    try
    {
		...
		if(pageDidLoad)
		{
			...
			// go to "Inactive Appointments" tab
			await profilePage.clickInactiveAppointmentsLink(loggingPrefix, driver);
			
			// click Edit Profile for Additional-Joint appointment on "Inactive Appointments" nav link (uses same xpath as "Active Appointments")
			...
			{
				await profilePage.selectAppointmentStatusAppointed(loggingPrefix, driver);
			...
			*/
			...
			// click Active Appointments link
			await profilePage.clickActiveAppointmentsLink(loggingPrefix, driver);
			
			// get Appointment ID under "Appointment Sets" link
			let jointAppointmentActiveAppointmentsExists = await profilePage.jointAppointmentActiveAppointmentsExists(loggingPrefix, driver);
			
			// check if Appointment ID under "Appointment Sets" link is same as 
			jointAppointmentActiveAppointmentsExists === true ? passFail = "passed" : passFail = "FAILED";
