...
const pageToTest = "EightYearClock";  // must use same key as used elsewhere (e.g. data/PAGE_PATHS)
...
exports.runPerm8YCTest = async (caller, environment, role, shouldSee) => {
    ...
    const testModule = "PERM-CLOCK-8YC";
    ...
    try
    {
		...
		if(pageDidLoad)
		{
		    ...
		    await selectedPage.clickEightYearClockLink(loggingPrefix, driver);
			...
			try
			{
				...
				permClockLib.logThis(loggingPrefix + "data has loaded in 8YC Profile table");
			}
			catch(error)
			{
				permClockLib.logThis(loggingPrefix + "NO data has loaded in 8YC Profile table");
				permClockLib.logThis(error);
		
				// test can't proceed without data
				rawResult = "ERROR: no 8YC records loaded on  Profile page";
			}


			//////////////////////////////////////////////////////////////////
			// 8YC LINK CLICK & BUTTON CHECK
			//////////////////////////////////////////////////////////////////
		...
		await permClockLib.printTestRunTime(loggingPrefix, testStartTime, testEndTime);
