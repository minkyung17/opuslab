runPermClock8YC = (caller, ottoEnvironment, ottoRole) =>
{
	const permClockLib = require("./PERM-CLOCK-LIB");
	const permClockTest = require("./PERM-CLOCK-8YC-TEST");
	
	
	///////////////////////////////////////////////////
	// ALL ROLES, ALL TESTS FOR PERM-CLOCK-8YC
	///////////////////////////////////////////////////
	
	// define all roles
	var allRolesForPERM_8YC =
	[
		"aa",
		"areaChair",
		"apoStaff",
	...
	logProperUsage = () =>
	{
		permClockLib.logThis("usage:   " + "node (relative dir/)PERM-CLOCK-8YC-CLI.js [ENV] [  [role(s)] --comma-separated, no spaces  ] [browser --optional] [headed --optional]");
		permClockLib.logThis("example: " + "node perm-clock/PERM-CLOCK-8YC-CLI.js TEST cap,oa,div chrome headed");
	}
	logProperUsage();
	...
	{
		if(argsEnvironment == null) //zmust be otto/scheduler
		{
	...
	const processedRole = permClockLib.handleRole(runType, allRolesForPERM_8YC, argsRole);
	...
	takeMyPERMClock8YCOrder = async (processedRole) =>
	{
		let shouldSee = rolesButtonVisibility.get(processedRole);
	
		await permClockTest.runPerm8YCTest(caller, argsEnvironment, processedRole, shouldSee) 
	}
	...
	// run tests
	permClockLib.executeRunOrder(runType, allRolesForPERM_8YC, allTestsForPERM_Clock, processedRole, "EightYearClock", takeMyPERMClock8YCOrder, overallStartTime);
}