...
describe('AdminCompAllocations Logic', () => {
  ...
  beforeAll(async (done) => {
    ...
    AdminCompLogic = new AdminCompAllocations(args);
...
//  });
//
 it('ensures "cloneEditData" returns correct cloned edit data', () => {
   let clonedRowDataFromFunction = AdminCompLogic.cloneEditData(pristineRowData);
   let clonedRowDataFromUtil = util.cloneObject(pristineRowData);
   assert.containsAllKeys(clonedRowDataFromFunction, clonedRowDataFromUtil);
 });

 it('ensures "setAddData" returns correct add data', () => {
  // sample add data
  let ahPathId = "4";
  let addData = {organizationName: "Arts and Architecture", organizationNameErrorMessage: null, 
   organizationNameHasError: false, title: "2", titleErrorMessage: null, titleHasError: false, unitName: "Arts and Architecture", 
   unitNameErrorMessage: null, unitNameHasError: false, year: "2020-2021", yearErrorMessage: null, yearHasError: false};
  let unitName = addData.unitName.split(":");

  // sample admin comp info
  let adminCompInfo = {academicHierarchyPathId: "4", academicYear: "2020-2021",
    adminCompID: null, department: undefined, division: undefined, fullTimeEquivalent: null,
    ninths: null, ninthsAmount: null, organizationName: "N/A", school: "Arts and Architecture", stipend: null,
    titleCodeId: "2", titleDescription: null};

  // get logic data
  let addDataFromFunction = AdminCompLogic.setAddData(addData, adminCompInfo);
  ...
  for(let each in adminCompInfo){
    if(each!=="adminCompID" && each!=="academicHierarchyPathId" && each!=="titleDescription"){
      ...
      testData[each] = dataToSet;
    }else if(each==="academicHierarchyPathId"){
      testData[each] = ahPathId;
    }
  }

  assert.containsAllKeys(addDataFromFunction, testData);
});
