...
describe('ActiveCases Logic', () => {
  ...
  beforeAll(async (done) => {
    ...
    ActiveCasesLogic = new ActiveCases(args);
  ...
  it('gets results from API', async (done) => {
    apiResults = await tableTests.getAPIResults(ActiveCasesLogic,
      {programTestData, getRowDataFromCache, api_cache_name, apiResults});
    done();
  });

  it('ensures the extracted rowData is Correct', () => {
    rowData = tableTests.extractRowData(ActiveCasesLogic,
      {apiResults});
  ...
  ****************************************************************************/
  ...
  it('ensures "getCsvUrlArgs" returns args needed data for Excel export', () => {
    let {csvPageName} = ActiveCasesLogic.getCsvUrlArgs();
    assert.equal(csvPageName, 'Active');
  });
